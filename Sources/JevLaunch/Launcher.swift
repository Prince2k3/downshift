...
/// Sends the proxy's swift-log output through `JevLog`, which writes to a file while a TUI
/// owns the terminal: a line printed under Claude Code or Codex would be drawn over.
public struct JevLogHandler: LogHandler {
    let log: JevLog
    public var metadata: Logger.Metadata = [:]
    public var logLevel: Logger.Level

    public init(log: JevLog) {
        self.log = log
...
/// The path skills and the status line run jev by.
public enum JevExecutable {
    /// The path jev was invoked by, made absolute, symlinks kept (a Homebrew path survives upgrades).
