...
/// How `jev claude` runs Claude Code (plan §5, §5a, §6).
public enum ClaudeLaunch {
    ...
    /// Whether the user configured a status line of their own; it takes priority over jev's.
    ...
    /// The forwarded arguments with jev's added first, so a trailing `-- prompt` stays last.
    /// When routing, `--model` is dropped: it became the baseline, and the session starts on
    /// the sentinel instead.
    public static func arguments(_ forwarded: [String], route: Bool, skillRoot: URL?, settingsFile: URL?) -> [String] {
        if isSubcommand(forwarded) { return forwarded }
        var added: [String] = []
        if let skillRoot { added += ["--add-dir", skillRoot.path] }
    ...
    /// The `--settings` file that adds jev's status line.
...
/// How `jev codex` runs the Codex CLI (plan §5a, §6).
public enum CodexLaunch {
    public static let provider = "jev"
    static let modelFlags: Set<String> = ["-m", "--model"]
    static let profileFlags: Set<String> = ["-p", "--profile"]

    /// The id `$jev-explain` finds this session's decisions under.
    public static func statusID(pid: Int32 = getpid()) -> String { "codex-\(pid)" }
