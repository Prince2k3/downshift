...
public struct UsageRecord: Codable, Equatable, Sendable {
    ...
    /// The model the user would have been on without dshift; nil when the user picked the model
    /// themselves (the baseline is then `model`).
    public var baseline: String?
    /// Whether dshift chose the model (the request asked for the router).
...
/// one file per month (`usage-2026-09.jsonl`, UTC). `dshift savings` prices it.
///
/// Several processes append at once (`dshift serve` and any `dshift claude`), so each record is one
...
public struct UsageLedger: Sendable {
    ...
    /// `~/Library/Application Support/downshift/usage` on macOS, `$XDG_STATE_HOME/downshift/usage` elsewhere.
    public static func standard(environment: [String: String] = ProcessInfo.processInfo.environment) -> UsageLedger {
        ...
        #if os(macOS)
        let root = home.appendingPathComponent("Library/Application Support/downshift", isDirectory: true)
        #else
        ...
        let root = state.appendingPathComponent("downshift", isDirectory: true)
        #endif
