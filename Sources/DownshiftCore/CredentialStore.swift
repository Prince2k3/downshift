...
/// Where `dshift setup` keeps host credentials: one generic-password item in the login keychain
...
public struct CredentialStore: Sendable {
    ...
    public static let keychain = CredentialStore(service: "dev.downshift")
    ...
    public func read() throws -> [String: String] {
        ...
        guard let values = try? JSONDecoder().decode([String: String].self, from: data) else {
            throw Failure(description: "the keychain item \(service) is not dshift's format")
        }
        return values
        #else
        return [:]
    ...
    public func write(_ values: [String: String]) throws {
        ...
        item[kSecAttrLabel as String] = "downshift host credentials"
        item[kSecAttrDescription as String] = "Jev host API keys (dshift setup)"
