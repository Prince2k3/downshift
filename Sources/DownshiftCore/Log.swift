...
public struct DownshiftLog: Sendable {
    ...
    public static var defaultFile: URL {
        FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/downshift/downshift.log")
    }
    ...
    /// File when stdout is a terminal, stderr otherwise; debug lines when `DSHIFT_DEBUG` is set.
    public init(settings: DownshiftSettings, file: URL = defaultFile) {
