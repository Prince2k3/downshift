...
/// List prices for the models dshift routes between, plus any the user adds or corrects in
...
public struct PriceTable: Sendable {
    ...
    /// `$XDG_CONFIG_HOME/downshift/prices.json`, else `~/.config/downshift/prices.json`.
    public static func overrideFile(environment: [String: String] = ProcessInfo.processInfo.environment) -> URL {
        ...
        return config.appendingPathComponent("downshift/prices.json")
    }
