...
/// A named way to reach Jev, built from environment variables (or the keychain, via `dshift setup`).
/// Adding a host is one entry here.
public struct HostPreset: Sendable {
    public var id: String
    public var summary: String
    /// What the preset reads, for `dshift hosts list`. Alternatives are joined with ` or `.
    public var variables: [String]
    /// What `dshift setup` asks for, saved under these variable names.
    ...
    /// The one secret `dshift setup --key-stdin` reads.
    public var secretField: SetupField? { setup.first { $0.kind == .secret } }
}

/// One value `dshift setup` asks for.
...
struct Env: Sendable {
    let environment: DownshiftEnvironment
...
public enum HostPresets {
    public static let cloudflare = HostPreset(
        ...
        variables: ["CLOUDFLARE_API_TOKEN_JEV or CLOUDFLARE_API_TOKEN", "CLOUDFLARE_ACCOUNT_ID",
                    "CLOUDFLARE_AI_GATEWAY (optional)", "DSHIFT_CLOUDFLARE_MODEL (optional)"],
        setup: [
        ...
        let model = env.first("DSHIFT_CLOUDFLARE_MODEL") ?? "typesafe/jev"
        let url: String
    ...
    public static let typesafe = HostPreset(
        ...
        variables: ["DSHIFT_API_KEY or TYPESAFE_API_KEY", "TYPESAFE_BASE_URL (optional)", "DSHIFT_MODEL (optional)"],
        setup: [SetupField("DSHIFT_API_KEY", "API key", .secret, help: "from your TypeSafe account")]
    ) { env in
        guard let key = env.first("DSHIFT_API_KEY", "TYPESAFE_API_KEY") else {
            return .failure(HostProblem(host: "typesafe", message: "DSHIFT_API_KEY is not set"))
        }
        ...
        return .success(JevHost(id: "typesafe", format: .systemOne, url: url, apiKey: key,
                                model: env.first("DSHIFT_MODEL") ?? "jev-latest"))
    }

    public static let vercel = HostPreset(
        id: "vercel", summary: "Vercel AI Gateway (typesafe-ai/jev)",
        variables: ["VERCEL_AI_GATEWAY_API_KEY_JEV or AI_GATEWAY_API_KEY", "DSHIFT_VERCEL_MODEL (optional)"],
        ...
        return .success(JevHost(id: "vercel", format: .vercel, url: "https://ai-gateway.vercel.sh/v4/ai/evaluation-model",
                                apiKey: key, model: env.first("DSHIFT_VERCEL_MODEL") ?? "typesafe-ai/jev"))
    }

    public static let openrouter = HostPreset(
        id: "openrouter", summary: "OpenRouter's Decisions endpoint (~typesafe/jev-latest)",
        variables: ["OPENROUTER_API_KEY", "DSHIFT_OPENROUTER_MODEL (optional)"],
        ...
        return .success(JevHost(id: "openrouter", format: .systemOne, url: "https://openrouter.ai/api/alpha/decisions",
                                apiKey: key, model: env.first("DSHIFT_OPENROUTER_MODEL") ?? "~typesafe/jev-latest"))
    }

    public static let custom = HostPreset(
        id: "custom", summary: "Any endpoint speaking one of the wire formats",
        variables: ["DSHIFT_BASE_URL (the full endpoint URL)", "DSHIFT_HOST_API_KEY",
                    "DSHIFT_TRANSPORT (systemone, workers-ai or vercel; default systemone)", "DSHIFT_MODEL (optional)"],
        setup: [
            SetupField("DSHIFT_BASE_URL", "Endpoint URL", help: "the full URL; https, or http to localhost"),
            SetupField("DSHIFT_HOST_API_KEY", "API key", .secret),
            SetupField("DSHIFT_TRANSPORT", "Wire format", .choice(JevHost.WireFormat.allCases.map(\.rawValue)), optional: true,
                       help: "Enter keeps systemone"),
        ]
    ) { env in
        guard let url = env.first("DSHIFT_BASE_URL") else {
            return .failure(HostProblem(host: "custom", message: "DSHIFT_BASE_URL is not set"))
        }
        if let problem = urlProblem(url) { return .failure(HostProblem(host: "custom", message: "DSHIFT_BASE_URL \(problem)")) }
        guard let key = env.first("DSHIFT_HOST_API_KEY") else {
            return .failure(HostProblem(host: "custom", message: "DSHIFT_HOST_API_KEY is not set"))
        }
        let transport = env.first("DSHIFT_TRANSPORT") ?? "systemone"
        guard let format = JevHost.WireFormat(rawValue: transport) else {
            ...
            return .failure(HostProblem(host: "custom", message: "DSHIFT_TRANSPORT must be one of \(known)"))
        }
        return .success(JevHost(id: "custom", format: format, url: url, apiKey: key, model: env.first("DSHIFT_MODEL") ?? "jev-latest"))
    }
    ...
    public static func host(_ id: String, environment: DownshiftEnvironment) -> Result<JevHost, HostProblem> {
    ...
    /// Which hosts to ask, in order: `--host`, then `DSHIFT_HOST` (a comma-separated list is a
    /// failover chain), then the first preset in `all` whose credentials are set.
    /// `none` turns routing off.
    public static func resolve(flag: String? = nil, environment: DownshiftEnvironment) -> HostResolution {
...
public struct HostResolution: Sendable {
    ...
    /// Where the choice came from: `--host`, `DSHIFT_HOST`, `credentials found`, ...
