...
/// What `dshift setup` and `dshift hosts remove` do to the stored credentials, as pure functions
...
public enum HostCredentials {
    ...
    /// `stored` without the host's variables, and without a `DSHIFT_HOST` that names only it.
    ...
    /// `DSHIFT_HOST` putting `id` first, keeping the rest of an existing failover list.
