...
struct SetupCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            ...
            `dshift hosts remove <host>` deletes one.
            Without a terminal:
              printf %s "$TOKEN" | dshift setup --host cloudflare --set CLOUDFLARE_ACCOUNT_ID=<id> --key-stdin
            After dshift is upgraded, macOS asks once whether dshift may read the item: choose Always Allow.
            """
    )

    @Option(help: "Host to set up (see `dshift hosts list`); asks when omitted.") var host: String?
    ...
    @Flag(name: .customLong("default"), help: "Make this host the one dshift asks first.") var makeDefault = false
    ...
    func run() async throws {
        ...
        let fileEnvironment = DownshiftEnvironment.load(credentials: nil)
        ...
        // What dshift will see: the shell environment, then the keychain, then env files.
        ...
        if first != preset.id {
            let current = first.map { "dshift asks \($0) first now" } ?? "dshift doesn't use \(preset.id) yet"
            if makeDefault || (interactive && Prompt.confirm("\n\(current). Use \(preset.id) first?", default: true)) {
                if process["DSHIFT_HOST"] != nil || fileEnvironment.values["DSHIFT_HOST"] != nil && updated["DSHIFT_HOST"] == nil {
                    print("DSHIFT_HOST is set in your environment or an env file; change it there to \(preset.id)")
                } else {
                    updated["DSHIFT_HOST"] = HostCredentials.preferring(preset.id, over: updated["DSHIFT_HOST"])
                }
            }
        }
        ...
        Self.restartProxy()
        environment = DownshiftEnvironment.load()
        let resolution = HostPresets.resolve(environment: environment)
        print("dshift asks: \(resolution.hosts.isEmpty ? "no host" : resolution.hosts.map(\.id).joined(separator: " then "))")
    }

    /// The LaunchAgent's proxy read its environment at start; restart it to pick up the change.
    static func restartProxy() {
        if LaunchAgent().restart() { print("Restarted the background proxy (dshift serve) to use it.") }
    }

    func choosePreset(interactive: Bool, stored: [String: String], environment: DownshiftEnvironment) throws -> HostPreset? {
        if let host {
        ...
        print("Jev picks the model for each prompt. Which host should dshift ask it through?")
...
/// Terminal prompts for `dshift setup`. Secrets are read with echo off and never printed.
enum Prompt {
...
struct StatusCommand: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: "Exits non-zero when an app is routed to a proxy that isn't listening. `dshift doctor` checks more.")
    ...
    func run() throws {
        ...
        if agentStatus.installed {
            ...
            how = "no LaunchAgent (`dshift serve install`)"
        }
        ...
        if let executable = agentStatus.executable, !FileManager.default.isExecutableFile(atPath: executable) {
            line("", "\(executable) is missing; run `dshift serve install`")
        }

        var problem = false
        for status in apps {
            guard status.enabled else {
                line(status.app.displayName, "not routed (`dshift apps enable`)")
                continue
            }
            let up = status.port.map { ProxyProbe.isListening(port: $0) } ?? false
            if !up { problem = true }
            var value = "routed to \(status.baseURL ?? "?")"
            if status.defaultProvider == false { value += ", dshift not the default provider" }
            if !up { value += ", NOTHING LISTENING: new sessions fail (`dshift serve install` or `dshift apps disable`)" }
        ...
        if resolution.hosts.isEmpty {
            line("Jev host", "none (\(resolution.source)); run `dshift setup`")
        } else {
        ...
        line("savings", savings.turns == 0 ? "no turns recorded in the last 7 days" : "\(savings.summary) in the last 7 days (`dshift savings`)")
