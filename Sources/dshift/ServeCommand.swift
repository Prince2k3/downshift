...
struct ServeCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            `dshift apps enable` points the apps at 127.0.0.1:\(AppsManager.defaultPort); this is what listens there.
              dshift serve                 run in the foreground (Ctrl-C to stop)
              dshift serve install         run it at login and keep it running (LaunchAgent)
              dshift serve uninstall       stop it and remove the LaunchAgent
            """,
    ...
    struct Run: AsyncParsableCommand {
        ...
        func run() async throws {
            if ProxyProbe.isListening(port: port) {
                throw AppsError("something is already listening on 127.0.0.1:\(port) (the LaunchAgent? see `dshift status`)")
            }
            Self.trimLogIfLarge()
            var logger = Logger(label: "dshift")
            ...
            // dshift. The apps don't pass --model, so that is the saved Claude Code model.
            let baseline = ClaudeSettings.savedModel().flatMap(ClaudeModel.tier(of:)) ?? .balanced
            let log = DownshiftLog(destination: .standardError, debugEnabled: debug)
            ...
            if resolution.hosts.isEmpty {
                logger.warning("no Jev host (\(resolution.source)); every conversation stays on its model. Run `dshift setup`.")
                router = nil
            } else {
            ...
            let codexEngine = RoutingEngine(adapter: codexAdapter,
                                            ...
                                            router: router, store: nil, log: log, ledger: ledger)
            ...
            try await ProxyServer.run(configuration, logger: logger) { [logger] bound in
                logger.info("dshift serve listening on http://127.0.0.1:\(bound)")
            }
        }
    ...
    struct Install: ParsableCommand {
        ...
        func run() throws {
            guard let executable = LaunchAgent.executablePath(invokedAs: CommandLine.arguments[0]) else {
                throw AppsError("can't tell where the dshift executable is; run it by its full path")
            }
            ...
            if LaunchAgent.isDevelopmentBuild(executable) {
                print("  note     development build copied from \(executable); run `dshift serve install` again after rebuilding")
            }
    ...
    struct Uninstall: ParsableCommand {
        ...
        func run() throws {
            ...
            if !routed.isEmpty && !force {
                ...
                throw AppsError("\(names) still route through the proxy, and would fail without it. Run `dshift apps disable` first, or pass --force.")
            }
