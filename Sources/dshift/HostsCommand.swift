...
struct HostsCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "hosts",
        abstract: "See and test the hosts dshift can reach Jev through.",
        discussion: """
    ...
    struct List: ParsableCommand {
        ...
        @Option(help: "Resolve as `dshift serve --host` would.") var host: String?
    ...
    struct Test: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            guard !hosts.isEmpty else {
                print("no configured host to test; run `dshift setup`")
                throw ExitCode(1)
            }
        ...
        static func probe(_ host: JevHost, environment: DownshiftEnvironment, prompt: String = probePrompt,
    ...
    struct Remove: ParsableCommand {
        static let configuration = CommandConfiguration(
            abstract: "Delete a host's credentials that `dshift setup` saved in the keychain.",
            discussion: "Env files and environment variables are yours to edit; this only touches the keychain.")

        @Argument(help: "Host id (see `dshift hosts list`).") var host: String
