...
/// The line `jev statusline` prints for Claude Code, which pipes the session as JSON on stdin
/// and shows whatever comes back. Claude Code's UI shows the model it asked for (the
/// sentinel), never the one jev routed to, so this is where the decision becomes visible.
