...
/// Jev's answer to the model question, plus the exact exchange for `jev explain`.
public struct RouteAnswer: Sendable {
    ...
    public init(choice: String, confidence: Double, metrics: JSONValue? = nil,
                request: JSONValue? = nil, response: JSONValue? = nil) {
        self.choice = choice
...
public actor RoutingEngine {
    ...
    /// Where every status is filed instead of the request's session: `jev codex` sets
    /// `JEV_CODEX_STATUS_ID` for the CLI, so `$jev-explain` finds the decision.
    nonisolated let statusSession: String?
    nonisolated let log: JevLog?
    ...
    ///     have been on without jev (plan §5a).
    ...
    ///   - statusSession: files every status under this id rather than the request's session.
    ...
    public init(adapter: any CLIAdapter = ClaudeCLI(), baseline: Tier, baselineModel: String? = nil,
                available: [Tier], router: JevRouter?, store: StatusStore?, statusSession: String? = nil,
                log: JevLog? = nil, ledger: UsageLedger? = nil, now: @escaping @Sendable () -> Date = Date.init) {
        ...
        self.router = router
        self.store = store
        self.statusSession = statusSession.flatMap { $0.isEmpty ? nil : $0 }
        self.log = log
        self.ledger = ledger
        self.now = now
    }
    ...
    public nonisolated func route(_ body: inout JSONValue, headerSession: String? = nil,
        ...
        let session = statusSession ?? (bodySession.isEmpty ? (headerSession ?? "") : bodySession)
        ...
        if decide, let prompt, !adapter.isExplainRequest(prompt) {
            ...
            var status: JSONObject = [
                ...
                "jev": answer.map { .object(["request": $0.request ?? .null, "response": $0.response ?? .null]) } ?? .null,
            ]
            status["at"] = .number(milliseconds())
            store?.writeDecision(status, session: session.isEmpty ? key : session)
        }
