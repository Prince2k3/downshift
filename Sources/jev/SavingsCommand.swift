...
struct SavingsCommand: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            ...
            ~/.config/jev/prices.json, e.g. {"models": {"my-model": {"input": 1, "output": 4}}}. \
    ...
    func run() throws {
        ...
        if let problem = pricing.problem { FileHandle.standardError.write(Data("jev: \(problem)\n".utf8)) }
    ...
    static func report(_ report: SavingsReport, since: Date?, period: String, baselines: [String],
        ...
        Swift.print("jev savings, \(window)\n")
        guard report.turns > 0 || report.jevCalls > 0 else {
            Swift.print("  Nothing recorded yet. Turns are recorded as they go through the jev proxy")
            Swift.print("  (`jev claude`, `jev codex`, or apps routed with `jev apps enable`).")
            return
        }
        ...
        line("without jev", amount(0) + baselineNote)
        line("with jev", amount(1))
        ...
        Swift.print("""
              ...
              the prompt cache, and that extra cost is already in "with jev".
            """)
    }
