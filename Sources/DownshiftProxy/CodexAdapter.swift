...
public struct CodexAdapter: CLIAdapter {
    ...
    /// `DSHIFT_CODEX_<TIER>_MODEL` overrides.
    ...
    /// Adds the downshift row to a `GET /models` response, copied from a real model so every
    ...
    public func addingJevModel(_ catalog: JSONValue) -> JSONValue? {
        ...
        entry["display_name"] = "Dynamic (Downshift)"
    ...
    public static func decisionEvents(_ outcome: RoutingOutcome, id: String = "dshift-\(UUID().uuidString.lowercased())") -> [JSONValue] {
        ...
        let text = outcome.reason.hasPrefix("jev-unavailable")
            ? "[Jev] unavailable; using \(outcome.model). Run `dshift setup` to configure a Jev host."
