...
/// The agent apps whose configuration `dshift apps` manages.
...
/// Where the managed files and dshift's own state live. Honours `CLAUDE_CONFIG_DIR` and
/// `CODEX_HOME` the same way the agents do, so dshift edits the file the agent actually reads.
public struct AppsLocations: Sendable {
    public var claudeSettings: URL
    public var codexConfig: URL
    /// Holds `apps.json` (what dshift changed) and `backups/`.
    ...
    public static func standard(
        ...
        #if os(macOS)
        let state = home.appendingPathComponent("Library/Application Support/downshift", isDirectory: true)
        #else
        let state = directory("XDG_STATE_HOME", default: ".local/state").appendingPathComponent("downshift", isDirectory: true)
        #endif
