...
/// `dshift apps enable/disable/status`: points the Claude and Codex apps at the local proxy by
...
public struct AppsManager: Sendable {
    ...
    public enum DisableOutcome: Sendable, Equatable {
        ...
        /// The file changed since enabling; only dshift's entries were removed.
    ...
    public struct Status: Sendable {
        ...
        /// Codex only: whether `model_provider = "downshift"` makes routing the default.
    ...
    public func enable(_ app: ManagedApp, port: Int = defaultPort, codexDefaultProvider: Bool = true) throws -> EnableOutcome {
        ...
        if let record = state[app.rawValue]?.objectValue {
            ...
            throw AppsError("\(app.rawValue) is already enabled with different settings; run `dshift apps disable --\(app.rawValue)` first")
        }
    ...
    public func disable(_ app: ManagedApp) throws -> DisableOutcome {
        ...
        guard let record = state[app.rawValue]?.objectValue else {
            // No record, but a dshift block may still be there (state deleted, or edited by hand).
