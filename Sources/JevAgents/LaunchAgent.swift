...
/// The per-user LaunchAgent that keeps `jev serve` running, so the proxy is up before the
/// apps start and comes back if it exits. `~/Library/LaunchAgents/dev.jev.serve.plist`,
/// loaded into the `gui/<uid>` domain, logging to `~/Library/Logs/jev/serve.log`.
public struct LaunchAgent: Sendable {
    public static let label = "dev.jev.serve"
    ...
    public init(home: URL = FileManager.default.homeDirectoryForCurrentUser,
        ...
        logURL = home.appendingPathComponent("Library/Logs/jev/serve.log")
        stagingDirectory = home.appendingPathComponent("Library/Application Support/jev/bin")
        domain = "gui/\(uid)"
    ...
    /// The path to put in the plist: the path jev was invoked by, made absolute but with
    /// symlinks kept, so a Homebrew install keeps pointing at `/opt/homebrew/bin/jev` across
    ...
    public func stage(executable: String) throws -> (path: String, changed: Bool) {
        ...
        let destination = stagingDirectory.appendingPathComponent("jev")
        let data: Data
    ...
    /// Restarts a loaded proxy so it rereads its environment (after `jev setup`). False when
