...
/// user (or the Codex app) wrote are never rewritten. Everything jev adds sits between marker
/// comments, and `disable` removes exactly those lines.
///
/// Two blocks are added:
/// - a `[model_providers.jev]` provider, appended at the end;
/// - optionally `model_provider = "jev"` among the top-level keys, which makes it the default.
...
///   to load its config at all.) CLI users can instead pass `-c model_provider=jev`.
public enum CodexConfigEdit {
    public static let beginMarker = "# >>> jev apps: managed by jev, remove with `jev apps disable --codex` >>>"
    public static let endMarker = "# <<< jev apps <<<"
    ...
    public static let defaultProviderBlock = [beginMarker, "model_provider = \"jev\"", endMarker]
    ...
    public static func enable(_ data: Data?, baseURL: String, defaultProvider: Bool) throws -> (Data, Previous) {
        ...
        if lines.contains(where: { $0 == beginMarker }) {
            throw AppsError("config.toml already has a jev block; run `jev apps disable --codex` first")
        }
        for line in lines where isTableHeader(line, named: ["model_providers", "jev"]) {
            throw AppsError("config.toml already defines `\(line.trimmingCharacters(in: .whitespaces))` outside jev's block; rename or remove it first")
        }
    ...
    public static func disable(_ data: Data, previous: Previous) throws -> Data {
        ...
        for line in splitLines(text) {
            if line == beginMarker {
                guard !inside else { throw AppsError("config.toml has a nested jev begin marker; remove the jev block by hand") }
                inside = true
            } else if line == endMarker {
                guard inside else { throw AppsError("config.toml has a jev end marker without a begin marker; remove it by hand") }
                inside = false
            } else if !inside {
                output.append(line)
            }
        }
        guard !inside else { throw AppsError("config.toml has an unterminated jev block; remove it by hand") }
