...
func codexTurn(_ text: String, cacheKey: String? = "main", tools: Bool = true) -> String {
    ...
    return #"{"model":"jev-router",\#(key)"reasoning":{"effort":"medium"},"input":[\#(additional){"role":"user","content":[{"type":"input_text","text":"\#(text)"}]}]}"#
}
...
@Suite struct CodexProxyTests {
    static let answer = RouteAnswer(
        ...
        metrics: ["taskComplexity": .number(0.82), "reasoningRequired": .number(0.91), "toolComplexity": .number(0.64), "contextSize": .number(0.31)],
    ...
    @Test func preservesAuthExtendsThePickerRoutesAndShowsTheDecision() async throws {
        ...
        try await withCodexProxy(engine: engine) { port, client, upstream in
            ...
            #expect(models.map { $0["slug"]?.stringValue } == ["jev-router", "gpt-5.6-terra", "gpt-5.6-sol"])
            ...
            #expect(status["prompt"]?.stringValue == "debug this race")
            #expect(status["jev"]?["request"]?["state"]?["request"]?.stringValue == "debug this race")
            #expect(status["history"]?.arrayValue?.count == 1)
            #expect(status["metrics"]?["reasoningRequired"]?.doubleValue == 0.91)
            ...
            let title = #"{"model":"jev-router","input":[{"type":"additional_tools","role":"developer","tools":[{}]},{"role":"user","content":"Generate a concise, single-line task title of at most 36 characters"},{"role":"user","content":"<environment_context><timezone>Asia/Calcutta</timezone></environment_context>"}]}"#
            ...
            #expect(afterTitle["history"]?.arrayValue?.count == 1)

            // Explaining reuses the conversation's model without asking Jev again.
            _ = try await post(client, port: port, path: "/codex/responses",
                               codexTurn("$jev-explain", tools: false), headers: codexHeaders)
            #expect(spy.requests.count == 1)
            #expect(await upstream.seen[3].body?["model"]?.stringValue == "gpt-5.6-sol")
            #expect(StatusStore(directory: dir).read(session: "cx1")?["metrics"]?["reasoningRequired"]?.doubleValue == 0.91)
        }
    }
    ...
    @Test func aModelThePickerChoseIsLeftAlone() async throws {
        ...
        let engine = RoutingEngine(adapter: CodexAdapter(), baseline: .balanced, available: Tier.allCases,
                                   router: spy.router, store: StatusStore(directory: dir))
        try await withCodexProxy(engine: engine) { port, client, upstream in
            let json = codexTurn("hi").replacingOccurrences(of: "jev-router", with: "gpt-5.6-terra")
            ...
            try await upstream.test(.live) { upstreamClient in
                ...
                try await proxy.test(.live) { proxyClient in
                    ...
                    try await WebSocketClient.connect(url: "ws://localhost:\(proxyPort)/codex/responses",
                        ...
                        let create = #"{"type":"response.create","model":"jev-router","input":[{"type":"additional_tools","role":"developer","tools":[{}]},{"role":"user","content":[{"type":"input_text","text":"debug this race"}]}]}"#
            ...
            try await upstream.test(.live) { upstreamClient in
                ...
                try await proxy.test(.live) { proxyClient in
                    ...
                    try await WebSocketClient.connect(url: "ws://localhost:\(proxyPort)/codex/responses",
                        ...
                        try await outbound.write(.text(create("jev-router")))
                        var completed = 0
                        for try await message in inbound.messages(maxSize: 1 << 22) {
