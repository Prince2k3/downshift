    ...
    var run: LaunchAgent.Launchctl {
        { [self] arguments in
            lock.withLock {
                ...
                switch arguments[0] {
                case "print": return loaded ? (0, "gui/501/dev.jev.serve = {\n\tstate = running\n\tpid = 4242\n}\n") : (113, "Could not find service")
                case "bootout":
                    defer { loaded = false }
...
struct LaunchAgentTests {
    ...
    init() throws {
        home = FileManager.default.temporaryDirectory.appendingPathComponent("jev-launchagent-\(UUID().uuidString)")
    ...
    @Test func plistContents() throws {
        let data = try LaunchAgent.plist(executable: "/opt/homebrew/bin/jev", port: 47821,
                                         log: URL(fileURLWithPath: "/Users/me/Library/Logs/jev/serve.log"))
        let plist = try #require(try PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any])
        #expect(plist["Label"] as? String == "dev.jev.serve")
        #expect(plist["ProgramArguments"] as? [String] == ["/opt/homebrew/bin/jev", "serve", "run", "--port", "47821"])
        #expect(plist["RunAtLoad"] as? Bool == true)
        #expect(plist["KeepAlive"] as? Bool == true)
        #expect(plist["StandardErrorPath"] as? String == "/Users/me/Library/Logs/jev/serve.log")
        // Deterministic, so an unchanged install can be detected by comparing bytes.
        #expect(data == (try LaunchAgent.plist(executable: "/opt/homebrew/bin/jev", port: 47821,
                                               log: URL(fileURLWithPath: "/Users/me/Library/Logs/jev/serve.log"))))
    }

    @Test func executablePathKeepsSymlinksAndResolvesRelativeAndPATH() throws {
        #expect(LaunchAgent.executablePath(invokedAs: "/opt/homebrew/bin/jev") == "/opt/homebrew/bin/jev")
        #expect(LaunchAgent.executablePath(invokedAs: ".build/debug/jev", currentDirectory: "/src/jev-router")
                == "/src/jev-router/.build/debug/jev")
        #expect(LaunchAgent.executablePath(invokedAs: "../bin/./jev", currentDirectory: "/usr/local/share") == "/usr/local/bin/jev")

        let bin = home.appendingPathComponent("bin")
        try FileManager.default.createDirectory(at: bin, withIntermediateDirectories: true)
        let tool = bin.appendingPathComponent("jev")
        FileManager.default.createFile(atPath: tool.path, contents: Data(), attributes: [.posixPermissions: 0o755])
        #expect(LaunchAgent.executablePath(invokedAs: "jev", searchPath: "relative:/nonexistent:\(bin.path)") == tool.path)
        #expect(LaunchAgent.executablePath(invokedAs: "jev", searchPath: "/nonexistent") == nil)
    }

    @Test func restartKickstartsOnlyALoadedJob() throws {
        #expect(!agent.restart())
        #expect(!launchctl.verbs.contains("kickstart"))
        try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821)
        #expect(agent.restart())
        #expect(launchctl.calls.last == ["kickstart", "-k", "gui/501/dev.jev.serve"])
    }

    @Test func installThenUninstall() throws {
        #expect(try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821) == .installed)
        ...
        #expect(launchctl.calls.contains(["enable", "gui/501/dev.jev.serve"]))
        ...
        #expect(status == .init(installed: true, loaded: true, pid: 4242, port: 47821, executable: "/opt/homebrew/bin/jev"))
    ...
    @Test func reinstallSameIsNoOpAndDifferentPortReloads() throws {
        _ = try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821)
        let before = launchctl.calls.count
        #expect(try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821) == .alreadyInstalled)
        #expect(launchctl.verbs[before...].allSatisfy { $0 == "print" })

        #expect(try agent.install(executable: "/opt/homebrew/bin/jev", port: 50000) == .replaced)
    ...
    @Test func plistPresentButUnloadedGetsLoaded() throws {
        _ = try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821)
        _ = launchctl.run(["bootout", "gui/501/dev.jev.serve"])
        #expect(agent.status().loaded == false)
        #expect(try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821) == .replaced)
    ...
    @Test func bootstrapIsRetriedThenReportsFailure() throws {
        ...
        #expect(throws: AppsError.self) { try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821) }
    }

    @Test func developmentBuildsAreStagedOutOfTheSourceTree() throws {
        let build = home.appendingPathComponent("src/jev-router/.build/debug")
        try FileManager.default.createDirectory(at: build, withIntermediateDirectories: true)
        let binary = build.appendingPathComponent("jev")
        FileManager.default.createFile(atPath: binary.path, contents: Data("v1".utf8), attributes: [.posixPermissions: 0o755])
        let staged = agent.stagingDirectory.appendingPathComponent("jev")
    ...
    @Test func installedBuildsRunInPlace() throws {
        #expect(!LaunchAgent.isDevelopmentBuild("/opt/homebrew/bin/jev"))
        #expect(!LaunchAgent.isDevelopmentBuild("/Users/me/.mint/bin/jev"))
        #expect(LaunchAgent.isDevelopmentBuild("/Users/me/src/jev/.build/arm64-apple-macosx/release/jev"))
        _ = try agent.install(executable: "/opt/homebrew/bin/jev", port: 47821)
        #expect(!FileManager.default.fileExists(atPath: agent.stagingDirectory.path))
    }

    @Test func aMissingDevelopmentBuildIsAClearError() {
        #expect(throws: AppsError.self) { try agent.install(executable: "/nonexistent/.build/debug/jev", port: 47821) }
        #expect(!FileManager.default.fileExists(atPath: agent.plistURL.path))
    }
