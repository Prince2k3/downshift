...
/// A throwaway home: settings.json, config.toml and the jev state directory under one temp dir.
struct Sandbox {
    let root: URL
    let manager: AppsManager

    init() throws {
        root = FileManager.default.temporaryDirectory.appendingPathComponent("jev-apps-\(UUID().uuidString)")
...
@Suite struct AppsManagerTests {
    ...
    @Test func claudeEditKeepsOrderAndIndent() throws {
        ...
        #expect(try box.read(.claude).contains(#""ANTHROPIC_CUSTOM_MODEL_OPTION_NAME": "Dynamic (Jev)""#))
    ...
    @Test func codexEditPutsProfileBeforeFirstTable() throws {
        ...
        let profile = try #require(lines.firstIndex(of: "model_provider = \"jev\""))
    ...
    @Test func noDefaultProviderLeavesTopLevelAlone() throws {
        ...
        let text = try box.read(.codex)
        #expect(!text.contains("model_provider = \"jev\""))
    ...
    @Test func claudeDisableAfterOutsideEditIsSurgical() throws {
        ...
        try box.write(.claude, claudeFixture)
        _ = try box.manager.enable(.claude)
        // The app rewrites settings.json while jev is enabled.
    ...
    @Test func claudePreviousValueIsRestoredAndChangedValueKept() throws {
        ...
        // The user replaced jev's value, so disable leaves it alone and says so.
    ...
    @Test func codexConflictsAreRefused() throws {
        let box = try Sandbox()
        try box.write(.codex, codexFixture + "[model_providers.jev]\nname = \"x\"\n")
