...
@Suite struct UsageLedgerTests {
    func ledger() throws -> UsageLedger {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("dshift-ledger-\(UUID().uuidString)")
    ...
    @Test func standardHonoursHome() {
        let ledger = UsageLedger.standard(environment: ["HOME": "/tmp/dshift-home"])
        #if os(macOS)
        #expect(ledger.directory.path == "/tmp/dshift-home/Library/Application Support/downshift/usage")
        #else
        #expect(ledger.directory.path == "/tmp/dshift-home/.local/state/downshift/usage")
        #endif
    }
}
