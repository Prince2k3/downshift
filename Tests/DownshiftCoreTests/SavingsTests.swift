...
@Suite struct PriceTableTests {
    ...
    @Test func overridesMergeAndMissingCacheRatesUseTheInputRate() throws {
        let home = FileManager.default.temporaryDirectory.appendingPathComponent("dshift-prices-\(UUID().uuidString)")
        let file = home.appendingPathComponent(".config/downshift/prices.json")
