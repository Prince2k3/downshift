...
struct TierTests {
    ...
    @Test func codexModelsAndOverrides() {
        ...
        let env = ["JEV_CODEX_STRONG_MODEL": "gpt-7-custom"]
        ...
        #expect(CodexModel.environmentKey(.fast) == "JEV_CODEX_FAST_MODEL")
    }
    ...
    @Test func longTierIsOptIn() {
        ...
        #expect(JevSettings(environment: .init(values: ["JEV_ALLOW_FABLE": "1"])).availableTiers.contains(.long))
        #expect(!JevSettings(environment: .init(values: ["JEV_ALLOW_FABLE": "true"])).availableTiers.contains(.long))
    }
}
